...
const tagger = read<{ n: number; slotAccuracy: number; colorAgreement: number; colorFamilyAgreement: number } | null>("public/bench/tagger.json", null);
...
const md = [
  ...
  tagger ? `Catalog tagger, on a held-out test set of ${tagger.n} pieces labelled blind before any tuning: right garment type ${pct(tagger.slotAccuracy)}, right colour family ${pct(tagger.colorFamilyAgreement)}, exact shade name ${pct(tagger.colorAgreement)}. Reference labels were made by a second AI model (Claude), not a person; the remaining colour misses are mostly printed sarees whose colour is itself debatable.` : "Tagger accuracy: not scored yet.",
  "",
  "## Limits",
  "",
